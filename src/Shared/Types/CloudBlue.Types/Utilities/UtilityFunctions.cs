...
public static class UtilityFunctions
{
    ...
    public static string ConstructMessage(List<string> messages)
    {
        ...
        {
            newMessages[idx++] = BreakDownWord(str);
        }
    ...
    public static string ConstructMessage(List<Errors> errorsLst)
    {
        var message = string.Empty;

        if (errorsLst.Count == 0)
        {
            return message;
        }

        var messages = errorsLst.Select(z => z.ToString())
            .ToList();
    ...
    public static string BreakDownWord(string word)
    {
        var regex = new Regex(@"
                (?<=[A-Z])(?=[A-Z][a-z]) |
                 (?<=[^A-Z])(?=[A-Z]) |
    ...
    public static T DeserializeJsonDocument<T>(JsonDocument document)
    {
        try
        {
            var obj = document.Deserialize<T>();
    ...
    public static string? GetCompositePhoneForDisplay(string? countryCode, string? areaCode, string? phone)
    {
        if (string.IsNullOrEmpty(countryCode) || string.IsNullOrEmpty(areaCode) || string.IsNullOrEmpty(phone))
        {
            return null;
    ...
    public static string ReplaceSpaceHolders<T>(string? targetString, string? anchorsJson, T sourceObject)
    {
        var returnString = string.Empty;

        if (string.IsNullOrEmpty(targetString) || string.IsNullOrEmpty(anchorsJson))
        {
            return returnString;
        }

        var anchors = DeserializeJsonString<AnchorItem[]>(anchorsJson);
        ...
        {
            var property = properties.FirstOrDefault(p => p.Name == anchor.PropertyName);
            ...
            returnString = returnString.Replace(anchor.AnchorText, value);
        }

        return returnString;
    }

    public static List<int> GetIntListFromString(string? stringValue)
    {
    ...
    public static List<long> GetLongListFromString(string? stringValue)
    {
        ...
        foreach (var str in arr)
        {
            if (long.TryParse(str, out var val) && val > 0)
            {
                retList.Add(val);
            }
        }
    ...
    public static DateTime GetUtcDateTime(DateTime date)
    {
        var cairoTimeZone = TimeZoneInfo.FindSystemTimeZoneById("Egypt Standard Time");

        return TimeZoneInfo.ConvertTimeToUtc(date, cairoTimeZone);
    }
